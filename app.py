...
def fetch_max_weight(sb: Client, exercise: str) -> float | None:
	res = (sb.table(TABLE).select("weight_kg").eq("exercise", exercise).order("weight_kg", desc=True).limit(1).execute())
...
def fetch_today(sb: Client) -> list[dict]:
	...
	res = (sb.table(TABLE).select("id, exercise, weight_kg, reps, created_at").gte("created_at", utc_str(start)).lt("created_at",
	                                                                                                                utc_str(end)).order("created_at").execute())
	return res.data or []


def fetch_all(sb: Client) -> list[dict]:
	res = (sb.table(TABLE).select("id, exercise, weight_kg, reps, created_at").order("created_at", desc=True).execute())
	return res.data or []
...
with tab_today:
	...
	exercise = st.selectbox("動作", EXERCISES, key="exercise")

	col_w, col_r = st.columns(2)
	weight = col_w.number_input("重量 (kg)", min_value=0.0, max_value=1000.0, value=20.0, step=2.5, format="%.1f", key="weight")
	...
	if not today_rows:
		...
		for name, sets in grouped.items():
			...
			for i, r in enumerate(sets, start=1):
				parts.append(f'<div class="setrow"><span class="idx">{i}</span>'
...
with tab_progress:
	...
	known_exercises = sorted({r["exercise"] for r in all_rows_chart} | set(EXERCISES))
